...
1. a _prior_ $$p(w)$$ distribution on the vector of parameters;

2. a _likelihood_ $$\mathcal{L}(w;D) := p(D|w) = \prod_{i} p(y_i|x_i, w)$$ (also called a _sampling_ or _data_ distribution); and

3. a _posterior_ $$p(w|D)$$, which is determined by the form of the above two distributions.
